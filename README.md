...
- **90 关 / 30 章剧情**：从山门初劫到道祖魔影，每章 3 关（l1 经典 → l2 异变 → l3 Boss），路径形态递进
- **7 种路径原型**：蛇形 / 十字汇聚 / Y 分流合流 / 回环 / 双桥渡河 / 窄道迷宫 / 螺旋，相邻章不重复；多路等长 ≤10% 铁律（ConfigLoader 启动校验）
- **三种异变玩法**：
  - **车道防守**（PvZ 式，ch5/10/15/20/25-l2）：敌从右侧直线压入守左端，每道一枚横扫符 + 波内灵晶拾取 + 清波奖大灵晶
  - **连续流夜袭**（末日式，ch7/12/17/22/27-l2）：全屏任意列出怪（提前 1s 光柱预警）+ 妖风变列 + **御剑守城**（v0.98：彻底无塔，真人沿墙线自由左右横移——**城墙与真人站位贴底最后两行**，各塔能力发射**真实弹道**（飞剑/符纸/枪/火球/电弧/冰晶）命中爆光+飘字掉血，攻击波内**击杀处爆塔符**、塔符自动飞入获得能力【6 系叠加共存、重复升级 ×1.4，本命飞剑千里剑气常驻】，点击敌人集火，稳定可通）+ **城墙布防**（塔只许筑于墙内），波间 0~1 秒无喘息
  - **三路经营战**（打小人式，ch14/23/30-l2）：中路打怪 + 两侧丹炉经营——收取伤害符、相邻同级合并升级（封顶 +60% 全塔伤害）
- **章节卡片选关**：30 章网格一览，打开自动定位到未通关章节
- **9 种塔 × 8 境界**：飞剑（单体）、符箓（对空）、长枪（穿透）、聚灵阵（光环增益）、火法（范围溅射）、雷法（链电）、寒冰（单体减速）、震地雷（埋雷，可伤遁地敌人），6 种攻击行为
- **45+ 种敌人 × 13+ 种机制**：飞行 / 护盾 / 受击回血 / 高甲 / 撞塔 / 隐身 / 分裂 / 遁地 / 闪避 / BOSS 魅惑召唤 / 狂暴 / 多阶段 / 精英
- **完整 meta 养成**：法宝 / 天命阶（VIP）/ 天赋树 / 皮肤 / 充值，三货币闭环
- **挑战系统**：每关 2 个挑战（速通/单流派/禁升级/禁光环/经济约束），复刷已通关关卡时全部同时开启，通关达成几个算几个，额外奖励贡献
...
│   ├── pure/                # 纯函数（可单测）：战斗/路径/经济/波次/目标/车道/锻炉
├── data/
...
└── ui/
    ├── Board.ts             # Canvas 渲染 + 输入（含车道关横扫符/灵晶拾取）
    ├── HUD.ts               # 状态栏
...
共 **443 个单元测试**，覆盖：战斗结算（护甲/护盾/回血/暴击/闪避/遁地）、路径几何、波次队列、目标选择、攻击策略（projectile/pierce/aoe/chain/mine）、Modifier 封顶与跨 stat 合并、BOSS 狂暴/魅惑/召唤、击退、经济、星级、解锁逻辑、挑战验证（5 种 kind）、无尽模式、装备/天赋/皮肤/塔配置、图鉴、**车道模式（横扫拦截/拾取确定性/收集幂等）**、**夜袭连续流（纵向横扫）**、**经营战（锻炉合并/收取/伤害注入）**、**御剑守城（鼠标跟位/塔符击杀爆出+自动飞入/弹道攻击+飘字掉血/集火/波间保留）**、**多路等长铁律校验**。
...
| 新塔加入主线 | `progressLevel.ts` 的 `TOWER_UNLOCK_TABLE` 加解锁条件 + `levelSelect.ts` 加 allTowerIds/iconMap |
...
| 塔 | 需通关数 | 解锁行为 |
|---|---|---|
| 飞剑修士、符箓修士 | 0（初始可用） | 单体投射 / 对空速射 |
| 长枪兵修、聚灵阵 | 1 | 横扫穿透 / 光环增益 |
| 寒冰修士 | 3 | 单体减速 |
| 火法修士 | 5 | 范围溅射 |
| 雷法修士 | 10 | 链电跳跃 |
| 震地雷 | 15 | 埋雷震荡，可伤遁地 |
